...
def main():
    ...
    parser.add_argument("--k", type=int, default=16,
        help="Training examples for each class, 0 means full set.")
    parser.add_argument("--task", type=str, nargs="+", 
        default=['QQP', 'MNLI', 'SNLI', 'QNLI'],
    ...
    parser.add_argument("--data_dir", type=str, default="dataset/original", help="Path to original data")
    parser.add_argument("--output_dir", type=str, default="dataset", help="Output path")
    parser.add_argument("--mode", type=str, default='k-shot', choices=['k-shot', 'k-shot-10x', 'full-ft'], help="k-shot or k-shot-10x (10x dev set) or all data for fine-tuning") 

    args = parser.parse_args()
    args.output_dir = os.path.join(args.output_dir, args.mode)
    if args.mode == 'full-ft':
        args.k = 0
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                ...
                with open(os.path.join(setting_dir, name), "w") as f:
                    for line in train_header:
                        f.write(line)
                    if k == 0:
                        assert args.mode == "full-ft"
                        for label in label_list:
                            for line in label_list[label][k:k+100]:
                                f.write(line)
                    else:
                        for label in label_list:
