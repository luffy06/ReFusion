...
sys.path.append("/disk3/xy/PROJECT/RetrievalLM/src/retriever")
...
def main(): 
    ...
    if 'prompt' in model_args.few_shot_type:
        if config.model_type == 'roberta':
            ...
            if not retrieval_args.enable_retrieval:
                model_fn = T5ForConditionalGeneration
            else:
                model_fn = RetrievalAugmentedT5
        else:
    ...
    if training_args.do_train:
        ...
        # if retrieval_args.peft_type != None:
        #     model = model_fn.from_pretrained(
        #         model_args.model_name_or_path,
        #         from_tf=bool(".ckpt" in model_args.model_name_or_path),
        #         config=config,
        ...
        #         model=model,
        #         model_id=training_args.output_dir,
        #     )
        # else:
    ...
    # with FileLock('log/results.lock'):
    #     with open('log/results.log', 'a') as f:
